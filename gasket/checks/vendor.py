...
def check_vendor(project_root: Path, *, vendor_dir: Path = Path("python_modules"), lock_file: Path = Path("uv.lock")) -> list[Finding]:
    ...
    if not vendor.exists():
        return [Finding(vendor, 1, "warning", "GSK030", "vendor directory missing; skip if project does not vendor Python packages")]
    for name in sorted(_imports_under(project_root / "src")):
        if not (vendor / name).exists() and not any(vendor.glob(f"{name}-*.dist-info")):
            findings.append(Finding(vendor, 1, "warning", "GSK031", f"imported package may be missing from vendor dir: {name}"))
    if (project_root / lock_file).exists() and vendor.stat().st_mtime < (project_root / lock_file).stat().st_mtime:
        findings.append(Finding(vendor, 1, "warning", "GSK032", "vendor directory older than lock file"))
    for ext in list(vendor.rglob("*.so")) + list(vendor.rglob("*.pyd")) + list(vendor.rglob("*.dylib")):
        findings.append(Finding(ext, 1, "error", "GSK033", "native extension file is not supported by Python Workers"))
    return findings
