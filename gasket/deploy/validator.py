...
def validate_ready(project_root: Path, *, wrangler_config: Path = Path("wrangler.jsonc"), required_secrets: list[str] | None = None) -> list[Finding]:
    findings = []
    ...
    if not config.exists():
        ...
        try:
            ...
            findings.append(Finding(config, 1, "error", "GSK101", f"wrangler config is not valid JSONC: {exc}"))
